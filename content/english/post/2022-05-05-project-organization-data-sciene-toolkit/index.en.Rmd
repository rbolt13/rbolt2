---
title: Project Organization (Part 1 and 2) - Data Sciene Toolkit
author: Randi Bolt
date: '2022-05-05'
slug: []
categories:
...
This is part 1 and 2 of my notes on Project Organization from [Data Science Toolkit](https://benkeser.github.io/info550/), by David Benkeser. 
...
Then in the terminal type: `Rscript R/test.R`, which should return the absolute path. 

Change line 3 from `here::here()` to `here::here("figs")`, save, and run `Rscript R/test.R`. This should show the absolute path of the "figs" folder. 

**Example 2**

In the terminal change to the R directory by typing: `cd R`. Check its correct by listing the items in that folder with `ls`. Then create a new directory in that folder called "R_subdir": `mkdir R_subdir`

Now create an R file simialr to the previous example and play around. 

Then when you're done remove the directory you created with `rm -rf R/R_subdir/` , `rm R/test.R`, `rm figs/barchart.png`
...
In the terminal confirm where you are with `pwd`, and then type `R`. This should show you what version of R you are using (and other things). Install renv, and then run `renv::init()` in the terminal. 

This should list out not only the packages we are using but packages that those packages may depend on. Then it is saving all of that information into a lockfile which is now saved into the project file. 

Notice that other files appeared as well, such as  ".Rprofile" which is a single line that says one line: 'source("renv/activate.R")'. When you open R is is looking for these .Rprofile files (similar to bash or zshell). Be careful messing with this file, because that creates issues with reproducibility. What the line `("renv/activate.R")' is doing is making R aware that we are in a project directory and there is a package library that's associated with that package directory. 
...
Close out R session in terminal with: `q()`, and then start a new R with 'R', and notice there is a different output, which recognizes our package environment. If you get errors loading packages then type `renv::restore()` which will install packages on your computer. 

Now look back at the rmd file `repor.Rmd` at lines 8 and 9, shown below. 
